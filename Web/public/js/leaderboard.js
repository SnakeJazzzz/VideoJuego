...
try
{
   ...
   const response1 = await fetch('http://127.0.0.1:3000/api/mapStats',{method: 'GET'});
    ...
    if(response1.ok)
    {
        ...
        let colors = getNrandom_colors(mapsName.length);
    ...
    const response2 = await fetch('http://127.0.0.1:3000/api/cardStats',{method: 'GET'});

    if(response2.ok)
    {
        console.log('Response is ok. Converting to JSON.')

        let results = await response2.json()
        ...
        let colors = getNrandom_colors(cardName.length);
