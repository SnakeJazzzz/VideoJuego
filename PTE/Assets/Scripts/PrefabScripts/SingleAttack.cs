...
public class SingleAttack : NPCSystem
{
    ...
    {
        while (npcController.closest != null) // Creates an infinite loop that's controlled by starting/stopping the coroutine
        {
            //Debug.Log("NPC Attacks!");
            yield return new WaitForSeconds(npcController.npcStats.attackCooldown);
            if (npcController.closest != null)
            {
                npcController.closest.GetComponent<IDamageable>().TakeDamage(npcController.npcStats.attack);
            }
